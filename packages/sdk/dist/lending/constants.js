...
export const LENDGUARD_PROGRAM_ID = new PublicKey("GQia1ewyLgtkgX7HSfuttJ42qNPpYJhUbxeyCPXtcJFR");
export const TOKEN_PROGRAM_ID = new PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
export const ASSOCIATED_TOKEN_PROGRAM_ID = new PublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL");
/** Devnet LGUSD mint (governed by the lending_pool PDA). */
export const LGUSD_MINT_DEVNET = new PublicKey("9NuCY56MCS8FcGZ1i3wjpzffjwb9mnAQdX4CwgNWzhpZ");
export const LGUSD_DECIMALS = 6;
export const LGUSD_SCALE = 1000000n;
export const PRICE_SCALE = 100000000n;
export const RAY = 1000000000000000000n;
export const COLLATERAL_DECIMALS = 1000000000n;
...
export function deriveLendingPoolPda(borrowAssetMint, programId = LENDGUARD_PROGRAM_ID) {
    return PublicKey.findProgramAddressSync([LENDING_POOL_SEED, borrowAssetMint.toBuffer()], programId);
}
export function deriveAdminPriceFeedPda(assetType, programId = LENDGUARD_PROGRAM_ID) {
    return PublicKey.findProgramAddressSync([ADMIN_PRICE_FEED_SEED, Buffer.from([assetType & 0xff])], programId);
}
export function deriveBorrowPositionPda(vaultPda, programId = LENDGUARD_PROGRAM_ID) {
    return PublicKey.findProgramAddressSync([BORROW_POSITION_SEED, vaultPda.toBuffer()], programId);
}
export function deriveVaultPda(owner, dwalletId, programId = LENDGUARD_PROGRAM_ID) {
    return PublicKey.findProgramAddressSync([VAULT_SEED, owner.toBuffer(), Buffer.from(dwalletId.slice(0, 32))], programId);
}
export function deriveRiskStatePda(vaultPda, programId = LENDGUARD_PROGRAM_ID) {
    return PublicKey.findProgramAddressSync([RISK_STATE_SEED, vaultPda.toBuffer()], programId);
}
/** ATA derivation matching `@solana/spl-token`'s `getAssociatedTokenAddressSync`. */
export function deriveAssociatedTokenAddress(owner, mint, allowOwnerOffCurve = false) {
