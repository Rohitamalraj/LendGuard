...
await pv.registerCollateralVault({
  ...
  },
  {
    ...
    description: "Ika network generates MessageApproval proving collateral is locked. ProofVault reads and validates it.",
    code: `// Ika dWallet generates proof
const proof = await pv.verifyCustodyProof(
  vaultId
);
...
await pv.triggerRiskCheck({
  vaultId,
  predicate: "backing >= 95%"
});
...
function highlightCode(line: string): string {
  return line
    .replace(/(pv|ProofVault|await|const|new|if|throw)/g, '<span class="text-foreground">$1</span>')
