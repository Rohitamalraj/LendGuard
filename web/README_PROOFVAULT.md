# ProofVault — Cryptographic Collateral Provenance for DeFi Lending on Solana

**"What would have saved KelpDAO $292 million."**

ProofVault is a cryptographic collateral verification layer for Solana lending protocols that uses **Ika dWallets** to prove cross-chain collateral is genuinely locked on origin chains (no bridge trust), and **Encrypt FHE** to evaluate protocol risk thresholds on encrypted data (preventing MEV front-running).
...
**ProofVault is that solution.**

---

## How ProofVault Works: 3-Layer Architecture

### Layer 1: Provenance Vault (Ika dWallet)

ProofVault proves that cross-chain collateral is **genuinely locked on its origin chain** using Ika's 2PC-MPC protocol.
...
ProofVault protects liquidation logic from MEV bots using **Encrypt's REFHE protocol**.
...
| Traditional Security | ProofVault |
|---|---|
...
ProofVault exposes rich on-chain events for full transparency:
...
- Real-time event stream for all ProofVault activities
...
Example query:
```
SELECT COUNT(*) as rejected_exploits FROM proofvault_events 
...
Torque MCP enables ProofVault to scale across multiple origin chains while maintaining single Solana verification layer.
...
Vault V1:
  ...
  → Single ProofVault on Solana manages all three collateral streams
```
...
┌──────────────────────────────────────────────────────────────────┐
│                   COVALENT GOLDRUSH                              │
│  Real-time indexing of all ProofVault events:                   │
...
```typescript
import { ProofVault } from "@proofvault/sdk";

const pv = new ProofVault({ connection, wallet, cluster: "devnet" });
const proof = await pv.verifyCustodyProof(dwalletId);
...
- ProofVault program reads proof
...
```bash
# 1. Clone and install
git clone https://github.com/your-org/proofvault
cd proofvault
npm install

# 2. Deploy program (if not already deployed)
cd programs/proofvault
anchor build
...
## @proofvault/sdk Package

Published on npm: `npm install @proofvault/sdk`
...
await pv.registerCollateralVault({
  dwalletId: new PublicKey("dW4llet..."),
  assetType: "BTC",
  amount: 50,
});

// 2. Verify custody proof
const proof = await pv.verifyCustodyProof(vaultId);
// Returns: { isValid: boolean, issuedAt: number, amount: number }

// 3. Deposit collateral
await pv.depositCollateral({
  vaultId,
  amount: 1_000_000, // in lamports
});

// 4. Check risk state (encrypted)
const isSafe = await pv.checkRiskState(vaultId);
// Returns: boolean (decrypted by program, not SDK)

// 5. Get vault state
const vault = await pv.getVault(vaultId);
// Returns: { owner, dwalletId, state, totalDeposited, lastProofTimestamp }

// 6. List all events (GoldRush-indexed)
const events = await pv.getVaultEvents(vaultId, {
  eventTypes: ["CollateralDeposited", "CircuitBreakerFired"],
  limit: 50,
});
```
...
All ProofVault events are automatically indexed by Covalent GoldRush. Query on-chain history:
...
// Get all ProofVault events in last 24 hours
...
## Comparing ProofVault to Existing Solutions

| Aspect | Bridge Oracle | Pyth | ProofVault |
|---|---|---|---|
...
# Program Deployment
PROOFVAULT_PROGRAM_ID=ProofVau1t...xyz
...
```json
{
  "scripts": {
    ...
    "sdk:build": "cd sdk/@proofvault/sdk && npm run build",
    "sdk:publish": "cd sdk/@proofvault/sdk && npm publish --tag alpha",
    "cpi:build": "cd crates/proofvault-cpi && cargo build",
    "cpi:publish": "cd crates/proofvault-cpi && cargo publish"
  }
}
```
...
Here's how Kamino would integrate ProofVault:

```typescript
// kamino-program/src/instructions/deposit_with_proofvault.rs
use proofvault_cpi::verify_custody_proof;

#[derive(Accounts)]
pub struct KaminoDepositWithProofVault<'info> {
    pub kamino_vault: Account<'info, KaminoVault>,
    pub proofvault_vault: Account<'info, ProofVaultVault>,  // Cross-program
    pub proofvault_program: Program<'info, ProofVaultProgram>,
    pub payer: Signer<'info>,
}

pub fn deposit_cross_chain_collateral(
    ctx: Context<KaminoDepositWithProofVault>,
    amount: u64,
) -> Result<()> {
    // 1. Verify collateral via ProofVault
    verify_custody_proof(
        ctx.accounts.proofvault_program.clone(),
        ctx.accounts.proofvault_vault.clone(),
    )?;
...
- [x] @proofvault/sdk published on npm (0.1.0-alpha)
- [x] proofvault-cpi crate available for on-chain integrations
...
- [x] Comparison table: ProofVault vs. existing solutions
...
- **GitHub:** [proofvault](https://github.com/your-org/proofvault)
- **npm SDK:** [@proofvault/sdk](https://www.npmjs.com/package/@proofvault/sdk)
- **Devnet Program:** `ProofVau1t...xyz`
- **GoldRush Dashboard:** [proofvault-metrics](https://dashboard.covalent.com/)
- **Figma Design:** [ProofVault UI Kit](https://figma.com/...)
...
- **Smart Contracts:** [Anchor Program Source](./programs/proofvault/src/)
- **Demo Video:** [YouTube Walkthrough](https://youtube.com/...)
- **Discord:** [ProofVault Community](https://discord.gg/...)
